...
export const HeaderNav: React.FC<HeaderNavProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-zinc-950/95 border-b border-zinc-800/80 backdrop-blur-md px-3 sm:px-5 py-2 flex items-center justify-between overflow-x-hidden">
      {/* Top Left: User Profile & Balance ($10.00 Bonus) */}
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div className={`relative w-8 h-8 sm:w-9 sm:h-9 rounded-full ${
              user.isAdmin || user.username === 'ADMIN'
                ? 'ring-2 ring-amber-300 shadow-[0_0_12px_rgba(245,158,11,0.7)]'
              ...
              <div className="flex items-center space-x-1">
                {user.isAdmin || user.username === 'ADMIN' ? (
                  <>
                    ...
                    <span className="text-[9px] px-1.5 py-0.2 rounded-full bg-amber-400/25 text-amber-200 font-black border border-amber-400/40 uppercase">
                      Admin
                    </span>
                  </>
                ) : (
      ...
      {/* Right Side: Fərdi Masa Yarat + Cashier / Deposit button with balance */}
      {user ? (
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="flex items-center space-x-1 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-zinc-950 px-2 py-1 rounded-lg text-[11px] font-black shadow-sm shadow-amber-500/20 transition-all active:scale-95 cursor-pointer whitespace-nowrap"
          >
          ...
          <button
            ...
            className="flex items-center space-x-1.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-700 text-zinc-200 px-3 py-1.5 rounded-xl text-xs font-black shadow-md transition-all active:scale-95 cursor-pointer"
          >
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="py-1.5 px-3 rounded-xl bg-zinc-900 hover:bg-zinc-850 border border-zinc-700 text-zinc-200 text-xs font-bold transition-colors"
          >
            {t.signin_btn}
          </button>
          <button
            ...
            className="py-1.5 px-3.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-zinc-950 text-xs font-bold shadow-md shadow-amber-500/20 transition-all"
          >
            {t.signup_btn}
          </button>
        </div>
